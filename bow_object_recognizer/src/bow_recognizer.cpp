...
//        if(apply_thresh && match_scores[i] < score_thresh) continue;
    ...
    if(best_matches.size())
    {
        ...
        for(size_t i = 0; i < best_matches.size(); i++)
        {
            cout << "Model " << best_matches[i].model_id << " has matching score: " << best_matches[i].score << "\n";
        }
...
void printParams()
{
    std::cout << "training dir: " << training_dir << "\n"
                ...
                "Apply threshold: " << apply_thresh << "\n"
                "Apply verification: " << apply_verification << "\n"
                "Limit matches: " << limit_matches << "\n";
}
