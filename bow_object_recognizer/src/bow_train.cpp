...
void printParams()
{
    std::cout << "training dir: " << training_dir.c_str() << "\n"
              ...
              << feature_descriptor.c_str() << "\n"
                                               "use partial views: "
              << use_partial_views << "\n"
                                      "knn search index params: "
              << knn_search_index_params << "\n";
}

void trainModelBowDescriptors(Model &training_model, std::map<string, BoWDescriptor> bow_descriptors)
{
    ...
    if (use_partial_views)
    {
        for (size_t j = 0; j < training_model.views.size(); j++)
        {
            ...
            vector<BoWDescriptorPoint> descriptors_vect;

            feature_estimator->loadFeatures(descr_file.str(), descriptors_vect);

            BoWDescriptor &view_bow_descr = bow_descriptors[bow_sample_id.str()];
    ...
    else
    {
        ...
        BoWDescriptor &model_bow_descr = bow_descriptors[bow_sample_id.str()];
        bow_extractor->compute(descriptors_vect, model_bow_descr);
    }
}

void trainBowDescriptors(BoWModelDescriptorExtractor::Ptr &bow_extractor, std::vector<BoWDescriptorPoint> &vocabulary)
{
    ...
    std::map<string, BoWDescriptor> bow_descriptors;
    ...
    for (auto &training_model : training_models)
    {
        ...
        if (use_partial_views)
        {
            for (auto &view_id : training_model.views)
            {
                ...
                BoWDescriptor &view_bow_descr = bow_descriptors[bow_sample_key.str()];
        ...
        else
        {
            ...
            BoWDescriptor &model_bow_descr = bow_descriptors[training_model.model_id];
...
int main(int argc, char **argv)
{
    ...
    vector<BoWDescriptorPoint> vocabulary;
